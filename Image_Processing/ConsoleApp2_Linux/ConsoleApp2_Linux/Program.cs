...
class Program
{
    static void Main()
    {
        ...
        Console.WriteLine("Chuyển đổi thành công2.");
    }
    static void ConvertBmpToJpeg(string bmpPath, string jpegPath)
    {
        // Kiểm tra xem tệp tin JPEG đã tồn tại chưa
        if (File.Exists(jpegPath))
        {
            // Xóa tệp tin JPEG cũ
            File.Delete(jpegPath);
        }
        // Tạo đối tượng Bitmap từ file BMP
        using (Bitmap bmpImage = new Bitmap(bmpPath))
        {
            // Lưu ảnh BMP dưới dạng JPEG xuống file
            bmpImage.Save(jpegPath, ImageFormat.Jpeg);
        }
    }
}
